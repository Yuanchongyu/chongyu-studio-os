(()=>{
  const S={data:null,month:null,week:null,cohort:null};
  ...
  const inPeriod=(x)=>{const d=x.txn_date||x.date;return (!S.month||monthKey(d)===S.month)&&(!S.week||weekKey(d)===S.week);};
  function scopeFor(m,month=S.month,week=S.week,cohort=S.cohort){
    const tx=m.tx.filter(x=>(!month||monthKey(x.txn_date)===month)&&(!week||weekKey(x.txn_date)===week));
    const allLessons=m.lessons.filter(x=>(!month||x.month===month)&&(!week||x.week===week));
    ...
    return {income,expense,net:income-expense,units,hours,directExpense,
      ...
      hourCoverage:units?coveredUnits/units:0};
  }
  ...
  function filterBar(m){
    const months=[...new Set(m.tx.map(x=>monthKey(x.txn_date)))].filter(Boolean).sort().reverse();
    const weeks=S.month?[...new Set(m.tx.filter(x=>monthKey(x.txn_date)===S.month).map(x=>weekKey(x.txn_date)))].sort().reverse():[];
    const cohorts=[...new Set(m.lessons.map(x=>x.cohort))].sort((x,y)=>x.localeCompare(y));
    let out='<div class="report-filters">';
    out+='<div class="report-filter"><label>'+(zh()?'月份':'MONTH')+'</label><select onchange="reportFilterMonth(this.value)"><option value="">'+(zh()?'全部时间':'All time')+'</option>'+months.map(x=>'<option value="'+esc(x)+'" '+(S.month===x?'selected':'')+'>'+esc(monthLabel(x))+'</option>').join('')+'</select></div>';
    out+='<div class="report-filter"><label>'+(zh()?'周':'WEEK')+'</label><select '+(!S.month?'disabled':'')+' onchange="reportFilterWeek(this.value)"><option value="">'+(zh()?'全部周':'All weeks')+'</option>'+weeks.map(x=>'<option value="'+esc(x)+'" '+(S.week===x?'selected':'')+'>'+esc(weekLabel(x))+'</option>').join('')+'</select></div>';
    ...
    out+='<div class="report-filter-actions"><button class="ghost" onclick="reportThisMonth()">'+(zh()?'本月':'This month')+'</button><button class="ghost" onclick="reportReset()">'+(zh()?'重置':'Reset')+'</button></div>';
    return out+'</div>';
  }
  function kpis(s){
    ...
    return '<div class="report-kpis">'
      +'<div class="report-kpi"><small>'+(zh()?'收入':'REVENUE')+'</small><strong>$'+money(s.income)+'</strong></div>'
      +'<div class="report-kpi"><small>'+(zh()?'支出':'EXPENSES')+'</small><strong>$'+money(s.expense)+'</strong></div>'
      +'<div class="report-kpi net"><small>'+(zh()?'净额':'NET')+'</small><strong>$'+money(s.net)+'</strong></div>'
      +'<div class="report-kpi"><small>'+(zh()?'平均收入 / 小时':'AVG REVENUE / HOUR')+'</small><strong>'+gross+'</strong><span class="sub">'+(zh()?'只按已填写课时计算':'Recorded-hour lessons only')+'</span></div>'
      +'<div class="report-kpi net"><small>'+(zh()?'平均课程净时薪':'AVG LESSON NET / HOUR')+'</small><strong>'+netHour+'</strong><span class="sub">'+(zh()?'收入减可直接归属支出':'Revenue minus attributable direct cost')+'</span></div>'
      ...
      +'</div>'
      +'<div class="report-quality"><span>'+(zh()?'课程':'Lessons')+' · '+s.units+'</span><span>'+(zh()?'已记录课时':'Hours logged')+' · '+money(s.hours)+' h</span><span>'+(zh()?'课时数据覆盖':'Hour coverage')+' · '+Math.round(s.hourCoverage*100)+'%</span></div>';
  }
  ...
  window.reportReset=()=>{S.month=null;S.week=null;S.cohort=null;renderLoaded();};
  window.reportSetMonth=m=>{S.month=m;S.week=null;renderLoaded();};
  window.reportSetWeek=w=>{S.week=w;renderLoaded();};
  window.reportSetCohort=c=>{S.cohort=c;renderLoaded();};
  window.reportFilterMonth=v=>{S.month=v||null;S.week=null;renderLoaded();};
  window.reportFilterWeek=v=>{S.week=v||null;renderLoaded();};
  window.reportFilterCohort=v=>{S.cohort=v?decodeURIComponent(v):null;renderLoaded();};
  window.reportThisMonth=()=>{const d=new Date();S.month=d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0');S.week=null;renderLoaded();};
