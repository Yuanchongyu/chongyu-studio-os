(()=>{
  ...
  function scope(m){
    const periodTx=m.tx.filter(inPeriod),periodLessons=m.lessons.filter(inPeriod),lessons=S.cohort?periodLessons.filter(x=>x.cohort===S.cohort):periodLessons;
    if(S.cohort){const income=lessons.reduce((s,x)=>s+x.income,0),expense=lessons.reduce((s,x)=>s+x.directExpense,0);return {income,expense,net:income-expense,units:lessons.reduce((s,x)=>s+x.units,0),hours:lessons.reduce((s,x)=>s+x.hours,0)};}
    const income=periodTx.filter(x=>x.direction==='income').reduce((s,x)=>s+Number(x.amount||0),0),expense=periodTx.filter(x=>x.direction==='expense').reduce((s,x)=>s+Number(x.amount||0),0);
    return {income,expense,net:income-expense,units:periodLessons.reduce((s,x)=>s+x.units,0),hours:periodLessons.reduce((s,x)=>s+x.hours,0)};
  }
  ...
  function breadcrumb(){
    let h='<div class="report-breadcrumb"><button class="'+(!S.month?'active':'')+'" onclick="reportReset()">'+(zh()?'全部时间':'All time')+'</button>';
    if(S.month)h+='<button class="'+(!S.week?'active':'')+'" onclick="reportSetMonth(\''+S.month+'\')">'+esc(monthLabel(S.month))+'</button>';
    if(S.week)h+='<button class="'+(!S.cohort?'active':'')+'" onclick="reportSetWeek(\''+S.week+'\')">'+esc(weekLabel(S.week))+'</button>';
    if(S.cohort)h+='<button class="active">'+esc(S.cohort)+'</button>';
    return h+'</div>';
  }
  function kpis(s){
    return '<div class="report-kpis"><div class="report-kpi"><small>'+(zh()?'收入':'REVENUE')+'</small><strong>$'+money(s.income)+'</strong></div><div class="report-kpi"><small>'+(zh()?'支出':'EXPENSES')+'</small><strong>$'+money(s.expense)+'</strong></div><div class="report-kpi net"><small>'+(zh()?'净额':'NET')+'</small><strong>$'+money(s.net)+'</strong></div><div class="report-kpi"><small>'+(zh()?'课程':'LESSONS')+'</small><strong>'+s.units+'</strong></div><div class="report-kpi"><small>'+(zh()?'记录课时':'HOURS LOGGED')+'</small><strong>'+money(s.hours)+' h</strong></div></div>';
  }
  ...
  function cohortCards(m){
    const base=m.lessons.filter(inPeriod),stats=cohortStats(base); if(!stats.length)return '';
  ...
  window.reportSetMonth=m=>{S.month=m;S.week=null;S.cohort=null;renderLoaded();};
  window.reportSetWeek=w=>{S.week=w;S.cohort=null;renderLoaded();};
