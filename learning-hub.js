(()=>{
  ...
  function allRecords(s){
    ...
    (s?.lessons||[]).forEach(r=>merged.set(Number(r.n||0),r));
    (s?.sessions||[]).forEach(r=>merged.set(Number(r.n||0),r));
    return [...merged.values()].sort((a,b)=>(b.n||0)-(a.n||0));
  }
  ...
  function skillRows(s){return (s.skills||[]).slice(0,6).map(k=>{const hasScore=k.score!=null&&Number.isFinite(Number(k.score));return `<div class="skill-compact-row"><span>${esc(tx(k.name))}</span><div class="bar"><i style="width:${hasScore?Math.min(100,Number(k.score)*10):100}%"></i></div><strong>${hasScore?`${Number(k.score)}/10`:(isZh()?'已观察':'Observed')}</strong></div>`}).join('')||`<div class="course-empty">${isZh()?'还没有足够证据形成能力信号':'Not enough evidence for skill signals yet'}</div>`;}
  ...
  function render(){
    ...
    content.dataset.learningHubRendering='1';content.innerHTML=p==='students'?studentView():courseView();delete content.dataset.learningHubRendering;setRole();setTimeout(()=>window.refreshStudentAvatars?.(),30);
  }
